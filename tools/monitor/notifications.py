...
Webhook 推送通道（payload 白名单 / Server酱 / 滑动窗口频控）已行为等价
上提到 core/notify.py 供多工具共享；本模块保留 Qt 适配器本体，并 re-export
既有导入面——page 与测试从 tools.monitor.notifications 导入的名字一个不少。
"""
...
from core.notify import (WEBHOOK_FORMATS, WEBHOOK_RATE_LIMIT,
