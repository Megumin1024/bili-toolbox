...
评论全量抓取 · 视频批量采集 · 实时数据监控 · 任务历史与运行诊断 · 本地数据分析 —— 免登录、开箱即用
...
| 工具 | 说明 |
|---|---|
| 💬 **评论抓取** | 输入动态 / 视频链接，通过 gRPC 游客通道抓取**全量评论**（主楼 + 楼中楼），自动生成精确分析报告（用户结构、时间分布、高频词、表情、点赞榜）与多 sheet Excel（含数据质量和字段说明）。支持断点续传 |
| 📊 **视频采集** | 视频 / 收藏夹 / 合集 / 系列 / txt 列表批量采集公开数据（播放、点赞、投币、收藏、分享、弹幕、评论），支持**定时追踪**模式并生成带质量统计的增速榜 Excel |
| 📡 **实时监控** | 监控视频作品或直播间：视频显示 8 项指标与趋势图表，直播间显示人气曲线、开播状态和房间信息；支持里程碑、停滞、突增、断连及开播/下播提醒，可选 Webhook / Server酱 |
...
| 💬 **弹幕抓取/分析** | 输入视频链接抓取弹幕，分析热词、高频弹幕、热点分钟，并导出含质量统计的密度分布 Excel 与原始 JSONL |
| 👥 **关系分析** | 导入本地粉丝 / 关注清单，分析互关、增减关注和月度变化；全程离线，不上传数据 |
| 🎥 **直播追踪** | 输入直播间号或链接，查看单次快照或按间隔追踪开播状态、人气、标题和分区，并导出 JSONL / Excel |
...
**评论抓取**：粘贴动态或视频链接（支持 `t.bilibili.com` / `opus` / `BV` / `av` / `b23.tv` 短链 / 纯数字动态 ID），选择输出目录后开始。产物：`评论分析_*.xlsx`（统计概览 / 分布统计 / 点赞 Top100 / 全量评论 / 数据质量 / 字段说明七个 sheet）、`分析报告.md`、`comments.jsonl` 原始数据。
...
**直播间监控**：在实时监控页选择「直播间」，输入房间号或链接即可查看人气和开播状态；视频监控与直播间监控共用同一套仪表盘和提醒规则。
...
**关系分析**：选择本地 CSV/JSON 粉丝或关注清单，工具会在本机计算互关、仅粉丝、仅关注、快照差异和月度分布，不访问网络也不会修改源文件。

**直播追踪**：输入直播间号或链接，选择单次快照或轮询追踪；结果包含开播状态、人气、标题和分区，可导出 JSONL / Excel。相邻轮次发生开播或下播变化时，可复用监控提醒通道。
